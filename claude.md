...
- Treat Alfred/environment inputs (`query`, workflow variables) as untrusted input.
...
- Verify icon file names referenced in `menu_filter.py` exist at project root.
...
- Keep build reproducible: source of truth is `src/`, package via Gulp, publish from `dist/`.
